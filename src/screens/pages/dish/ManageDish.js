...
const ManageDish = ({ navigation, route }) => {
  ...
  const handleAjoutPress = () => {
   ...
   useEffect(() => {
     const fetchPlats = async () => {
       try {
         const response = await fetch("http://172.22.231.205:9006/user/listePlatUser/655f5ef0965e4b2718a351bd");
  ...
  const handleUpdate = () => {
       navigation.navigate('PutDish');
  };

  const handleDelete = async (platId) => {
    try {
      const response = await fetch(`http://172.22.231.205:9006/user/delete/plat/${platId}`, {
        method: 'DELETE',
        headers: {
  ...
  const handleValider = () => {
          try {
            const response = fetch('http://172.22.231.205:9006/user/add/plat/655f5ef0965e4b2718a351bd', {
              method: 'POST',
              headers: {
                'Content-Type': 'application/json',
              },
      ...
      <FlatList
        ...
        renderItem={({ item }) => (
          <View style={styles.itemContainer}>
            ...
            <TouchableOpacity style={styles.buttonContainerMod} onPress={() => navigation.navigate('PutDishScreen', { recetteIds: item._id })}>
              <Text style={styles.buttonText}>Modifier</Text>
            </TouchableOpacity>
            <TouchableOpacity style={styles.buttonContainerSup} onPress={() => handleDelete(item._id)}>
